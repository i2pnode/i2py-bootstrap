...
class ToDo():
    ...
    updateFunction = 0
    consoleOutput = 0
    smallestSize = 0 #remove unnecessary line breaks and the like.
    customHelp = 0
parser = argparse.ArgumentParser()
parser.add_argument('-s', '--start',dest='start',help='default startup sequence',action='store_true')
parser.add_argument('-R', '--restart',dest='restart',help='power cycle services',action='store_true')
parser.add_argument('-K', '--shutdown',dest='shutdown',help='Shutdown router and kill all other services',action='store_true')

class i2pd():
    def start():
        print('Starting i2pd service..')
        os.system('i2pd --service --daemon')
    def shutdown():
        print(' ')
